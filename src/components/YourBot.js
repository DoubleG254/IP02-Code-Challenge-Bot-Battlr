...
function YourBot({avatar_url, name, catchphrase, health, damage, armor ,bot_class,id,removeBot,deleteBot}) {
  function handleClick(e){
    e.preventDefault()
    fetch(`https://mybots1.onrender.com/mybots/${id}`,{
      method:"DELETE"
    })
    removeBot(id)
  }
  function onDelete(e){
    ...
    fetch(`https://bots-fcbv.onrender.com/bots/${id}`,{
      method:"DELETE"
    })
    removeBot(id)
    deleteBot(id)
    ...
    <div onClick={handleClick} className="w-52 border border-black flex flex-col bg-gray-200 p-4 m-4">
      ...
      <button onClick={onDelete}>
        <svg
          ...
          strokeWidth={1.5}
          stroke="currentColor"
          className="w-6 h-6 text-red-600 hover:bg-red-600 hover:text-black"
        >
          <path
            strokeLinecap="round"
